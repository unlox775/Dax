 ...
 *
 * <code>
 * class Stark__ORM__DBHProvider extends Stark__ORM {
 ...
 * relations to other tables (and Stark__ORM objects that represent
 * those tables).  Stark ORM then provides standard get, set,
 ...
 * @package Stark__ORM
 * @version $Id: Stark__ORM.class.php,v 1.3 2009/10/30 18:55:35 dave Exp $
 */
class Stark__ORM {
    /**
     ...
     * are accesses, but they contain actual Stark__ORM object(s).
     ...
     * up with their own Stark__ORM-derived class in some file.  All
    ...
    protected $__relations = array();
    public $__dbh = null;
    protected $__dbh_type = null;
    public $__pk_values = array();
    protected $__data = array();
    protected $__relation_data = array();
    protected $__columns_to_save = array();
    protected $__object_forward = null;
    ...
    public function __construct($pk_value = array(), $select_star_data = null, $sync_dbh = null,$sync_select_dbh = null) {
        ...
        if ( is_array( $pk_value ) ) {
            foreach ($this->__primary_key as $col) { $this->__pk_values[$col] = array_shift($pk_value); }
        }
...
#               bug("USING CACHE: ".$this->__cache_key);
     ...
     *
     * Call this if you suspect that anything Non-Stark__ORM
    ...
    public function get($arg1) {
        if ( $this->__object_forward !== null ) return $this->do_object_forward_method(__FUNCTION__);
        $caller_funcs_to_ignore = array('get','get_all','__get','get_relation','__isset','set','__set','__unset','exists','call_user_func_array','do_object_forward_method');
        //error_log("BBB". trace_blame_line($caller_funcs_to_ignore));
        START_TIMER('Stark__ORM getX', self::$_SQL_PROFILE);

        ###  Quick return for vast common case
        if ( $this->__obj_state != 'deleted' && count( func_get_args() ) == 1 && isset($arg1) && (! is_array($arg1)) && isset($this->__data[$arg1]) ) { END_TIMER('Stark__ORM getX', self::$_SQL_PROFILE);  return $this->__data[$arg1]; }
        ...
        foreach ($columns as $col) {
            if ( !    isset($this->__schema[$col])
                 && ! isset($this->__relations[$col])
                ) {
...
#        if ( ! is_null($this->__obj_state) && $this->__obj_state != 'active' ) {
#            trigger_error( 'Call to get() on a "'. get_class($this) .'" object that is "'. $this->__obj_state .'" in '. trace_blame_line($caller_funcs_to_ignore), E_USER_ERROR);
#            return false;
#        }
        ...
        END_TIMER('Stark__ORM getX', self::$_SQL_PROFILE);
    ...
    public function set($to_set) {
        ...
        ###  Get all the data if it's not already cached...
        if ( empty( $this->__data ) ) $this->get($this->__primary_key[0]);

#        ###  Must be installed and active
#        if ( ! $this->exists() && $this->__obj_state != 'active' ) {
#            trigger_error( 'Call to set() on a "'. get_class($this) .'" object that is "'. $this->__obj_state .'" in '. trace_blame_line($caller_funcs_to_ignore), E_USER_ERROR);
#            return false;
#        }
    ...
    public function save() {
        ...
        ###  Must be installed and active
        if ( ! $this->exists() && $this->__obj_state != 'active' ) {
#            trigger_error( 'Call to save() on a "'. get_class($this) .'" object that is "'. $this->__obj_state .'" in '. trace_blame_line($caller_funcs_to_ignore), E_USER_ERROR);
#            return false;
            return $this->create(array());
        }
        if ( empty( $this->__columns_to_save ) ) return true;
        
        if ( is_null( $this->__dbh ) ) $this->load_dbh();
        if ( ! $this->pre_save_handler($this->__columns_to_save) ) return false;
        
        ###  Update the values
        $values = array();
        ksort($this->__columns_to_save); # help in query caching
        $set_clause = array();  foreach (array_keys($this->__columns_to_save) as $col) { $set_clause[] = "$col = ?";  $values[] = $this->__data[$col]; }
        $pk_where = array();    foreach ($this->__primary_key                 as $col) { $pk_where[]   = "$col = ?";  $values[] = $this->__pk_values[$col]; }
        ...
        if ( ! $this->post_save_handler($this->__columns_to_save) ) return false;
        
        ###  Reset the to-be-saved queue
        $this->__columns_to_save = array();
        if ( isset( $this->__data['__old__'] ) ) unset( $this->__data['__old__'] );
...
#        ###  Must be installed and active
#        if ( ! is_null($this->__obj_state) && $this->__obj_state != 'active' ) {
#            trigger_error( 'Call to get_relation() on a "'. get_class($this) .'" object that is "'. $this->__obj_state .'" in '. trace_blame_line($caller_funcs_to_ignore), E_USER_ERROR);
#            return false;
#        }
        ###  Return the cached answer if it's there
        if ( isset( $this->__relation_data[$name] ) ) return( $force_ordered_list && is_array( $this->__relation_data[$name] ) ? array_values( $this->__relation_data[$name] ) : $this->__relation_data[$name] );
        ...
        if ( $rel['relationship'] == 'has_one' ) {
            ...
            if ( ! empty($rel['custom_query_hook']) ) {
                ...
                $values = array();
                $where = array();  foreach ($fk_columns as $i => $col) { $where[] = "$col = ?";  $values[] = ($join_vals) ? $join_vals[$i] : $this->__pk_values[$this->__primary_key[$i]]; }
...
#        if ( $this->__object_forward !== null ) return $this->do_object_forward_method(__FUNCTION__);
    ...
    public function create($to_set = array()) {
        ...
        if ( $this->exists() || $this->__obj_state != 'not_created' ) {
            trigger_error( 'Call to create() on a "'. get_class($this) .'" object that is "'. $this->__obj_state .'" in '. trace_blame_line($caller_funcs_to_ignore), E_USER_ERROR);
            return false;
        }

        ###  Merge in any params they have previously set()
        foreach (array_keys($this->__columns_to_save) as $col) { if ( ! array_key_exists( $col, $to_set ) ) $to_set[$col] = $this->__data[$col]; }

        ###  Check in advance that we will be able to get the new Primary Key values...
        if ( $this->__dbh_type == 'pg' ) {
            foreach ($this->__primary_key as $col) {
                if ( ! isset($to_set[$col]) && ! isset($this->__column_sequences[$col]) ) {
                    trigger_error( 'In create, could not proceed because I would not be able to get complete primary key after insert.  You probably need to define a "column_sequences" definition in your Stark__ORM object'. get_class($this) .'::'. $col . ' in '. trace_blame_line($caller_funcs_to_ignore), E_USER_ERROR);
                }
            }
        }
        ...
        $sql = "INSERT INTO ". $this->__table ." (". join(',', $fields) .")
                VALUES (". join(',', $q_marks) .")
                  "; #"
        $sth = $this->dbh_do_bind($sql, $values);

        ###  Populate $this->__pk_values
        foreach ($this->__primary_key as $col) {
            if ( isset($to_set[$col]) ) {
                $this->__pk_values[$col] = $to_set[$col];
            }
            else if ( $this->__dbh_type == 'pg' && isset($this->__column_sequences[$col]) ) {
                $this->__pk_values[$col] = $this->dbh()->lastInsertId($this->__column_sequences[$col]);
            }
            else if ( $this->__dbh_type != 'pg' ) {
                $this->__pk_values[$col] = $this->dbh()->lastInsertId();
            }
        ...
        $this->__data = array(); //  Force them to re-get() to get the default values, PKey, etc.
        $this->__obj_state = null;
        $this->__columns_to_save = array();
        if ( isset( $this->__data['__old__'] ) ) unset( $this->__data['__old__'] );
        
        ###  Add to cache
        $pk_string = array();  foreach ($this->__primary_key as $col) { $pk_string[] = $this->__pk_values[$col]; }
        $this->__cache_key = get_class($this). '||--||'. $this->__table .'||--||'. join('||--||', $pk_string);
        $Stark__ORM_OBJECT_CACHE[ $this->__dbh ][$this->__cache_key] = $this;
    ...
    public function delete() {
        ...
        ###  Must be installed and active
        if ( ! is_null($this->__obj_state) && $this->__obj_state != 'active' ) {
            trigger_error( 'Call to delete() on a "'. get_class($this) .'" object that is "'. $this->__obj_state .'" in '. trace_blame_line($caller_funcs_to_ignore), E_USER_ERROR);
            return false;
        }
        ...
        unset($this->__data, $this->__dbh, $this->__columns_to_save );
        $this->__obj_state = 'deleted';
    ...
    public function pre_load_sub_relations( $name, $sub_relations_to_load, $force_ordered_list = true ) {
        ...
        ###  Must be installed and active
        if ( ! is_null($this->__obj_state) && $this->__obj_state != 'active' ) {
            trigger_error( 'Call to pre_load_sub_relations() on a "'. get_class($this) .'" object that is "'. $this->__obj_state .'" in '. trace_blame_line($caller_funcs_to_ignore), E_USER_ERROR);
            return false;
        }
...
#        if ( isset( $this->__relation_data[$name] ) ) return( $force_ordered_list && is_array( $this->__relation_data[$name] ) ? array_values( $this->__relation_data[$name] ) : $this->__relation_data[$name] );
        ...
        $values = array();
        $where = array();  foreach ($fk_columns as $i => $col) { $where[] = "$alias.$col = ?";  $values[] = $this->__pk_values[$this->__primary_key[$i]]; }
        if ( ! empty($rel['custom_where_clause']) ) $where[] = $rel['custom_where_clause'];
 ...
 * validation.inc.php
 * 
 * This file contains data validation routines, especially for use by the {@link Stark__ORM} class
 * @author Dave Buchanan <dave@StarkMVC.com>
 * @package StarkMVC
 * @version $Id: Stark__ORM.class.php,v 1.5 2010/09/28 21:20:56 dave Exp $
 */
...
function form_extract($form = null) {
    ...
    $prefix = '';
    $tmp = array_values($cols);
    if ( count( $cols ) == 1 &&   is_array($tmp[0]) ) { $cols = array_shift($tmp); };
    if ( count( $cols ) == 2 &&   is_array($tmp[0])
         &&                     ! is_array($tmp[1]) ) { list($cols,$prefix) = $tmp; };
...
function validate($form = null) {
    ...
    $cols = array_slice( func_get_args(), 1 );
    $prefix = '';
    if ( ($tmp = end($cols)) && is_object( $tmp ) ) $val_object = array_pop($cols);
    ###  Schema will be an array of arrays as the last param...
    if ( count( $cols ) > 1
         && ($tmp = end($cols))         && is_array($tmp)
         && ($tmp = array_values($tmp)) && is_array($tmp[0])
    ...
    $tmp = array_values($cols);
    if ( count( $cols ) == 1 &&   is_array($tmp[0]) ) { $cols = $tmp[0]; };
    if ( count( $cols ) == 2 &&   is_array($tmp[0])
         &&                     ! is_array($tmp[1]) ) { list($cols,$prefix) = $tmp; };
 ...
 * coming from the schema definition of a {@link Stark__ORM}
 * derived object).
 *
