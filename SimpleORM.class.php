 ...
 * @see $schema
 * @see $relations
 * @author Dave Buchanan <dave@dax.com>
 * @package TSANet
 * @version $Id: SimpleORM.class.php,v 1.2 2010/09/29 16:00:38 daxd Exp $
 */
class SimpleORM {
    /**
    ...
    public function __construct($pk_value = array(), $select_star_data = null) {
        ...
        $this->db_type =& $this->provide_db_type();
...
#            bug("USING CACHE: ".$this->cache_key);
    ...
    public function get($arg1) {
        ...
        ###  Allow '*' to indicate return all columns
        if ( $arg1 === '*' ) { $cols_from_array = true;  $columns = array_keys($this->schema); };
        ...
        if ( $this->table && empty( $this->data ) && ( is_null($this->state) || $this->state == 'active' ) ) {
            ...
            $sth = dbh_query_bind($sql, $values, $this->dbh);
        ...
        ###  Get the column names too if get through '*'
        if ( $arg1 === '*' ) $return_ary = array_combine($columns,$return_ary);
    ...
    public function save() {
        ...
        $sql = "UPDATE ". $this->table ."
                 ...
                 WHERE ". join(' AND ', $pk_where) ."
                  "; #"
        $sth = dbh_do_bind($sql, $values, $this->dbh);
    ...
    /**
     * get_relations to get a list of available relations
     * get_ids to get the unique identifier of the object
     * get_columns to get a list of columns
     */
...
function do_validation( $col, $value, array $valhash ) {
    ...
    ###  If there IS a value, validate it...
        ...
        ###  Max length
        if    ( ! empty($valhash['maxlength']) && strlen($value) > $valhash['maxlength'] 
            ) { return array( false, $value, array( array($name." may not be longer than ". $valhash['maxlength'] ." characters",'too_long')) ); }
        ###  Min length
        else if ( ! empty($valhash['minlength']) && strlen($value) < $valhash['minlength']
...
function dbh_query_bind( $sql ) {
    global $dbh;
    $use_dbh = $dbh;
    START_TIMER('dbh_query_bind', SQL_PROFILE);
    $bind_params = array_slice( func_get_args(), 1 );
    ###  Allow params passed in an array or as args
    if ( is_a( $bind_params[ count($bind_params) - 1 ], 'PDO' ) ) $use_dbh = array_pop($bind_params);
...
#    if (SQL_DEBUG) trace_dump();
    ...
    if (SQL_DEBUG) bug($sql, $bind_params);
    try { 
    ...
    END_TIMER('dbh_query_bind', SQL_PROFILE);
    return $sth;
}
/**
...
function dbh_do_bind( $sql ) {
    global $dbh;
    $use_dbh = $dbh;
    START_TIMER('dbh_do_bind', SQL_PROFILE);
    $bind_params = array_slice( func_get_args(), 1 );
    ###  Allow params passed in an array or as args
    if ( is_a( $bind_params[ count($bind_params) - 1 ], 'PDO' ) ) $use_dbh = array_pop($bind_params);
    if ( count( $bind_params ) == 1 && is_array(array_shift(array_values($bind_params))) ) { $bind_params = array_shift(array_values($bind_params)); };
    ...
    reverse_t_bools($bind_params);
    if (SQL_DEBUG || SQL_WRITE_DEBUG) bug($sql, $bind_params);
    try { 
        $sth = $use_dbh->prepare($sql);
        $rv = $sth->execute($bind_params);
    } catch (PDOException $e) {
    ...
    END_TIMER('dbh_do_bind', SQL_PROFILE);
    return $rv;
}
