 ...
 *   <li>START_TIMER(), END_TIMER(), PAUSE_TIMER(), RESUME_TIMER() - Benchmarking functions described below 
 * </ul>
 * 
 ...
 * a piece in the middle of your function. In the main
 ...
 * @package DAX
 * @version $Id: debug.inc.php,v 1.1 2010/08/05 03:48:23 daxd Exp $
 */

$ENV_MODE = ( ($DAX_MODE_BY_TRUSTED_IP)
              ? ( ( array_search($DAX_TRUSTED_ALPHA_IPS, $_SERVER['REMOTE_ADDR']) !== false ) ? 'alpha' : 'live' )
              : ( isset($_SERVER['ENV_MODE']) ? $_SERVER['ENV_MODE'] : 'alpha')
              );

$BUG_ON   = ( preg_match('/^alpha/', $ENV_MODE) ) ? true : false;
function on_alpha() { global $ENV_MODE;  return( ( preg_match('/^alpha/', $ENV_MODE) ) ? true : false ); }
if(isset($_SERVER['FORCE_DEBUG_OFF']) && $_SERVER['FORCE_DEBUG_OFF']) $BUG_ON = false;
// Debugging Setup
...
function RESUME_TIMER($tag, $THIS_ON = true) {
    ...
    if ( ! $BUG_ON || ! $THIS_ON ) return true;
    if ( ! isset($BUG_TIMERS[$tag]) || ! is_array($BUG_TIMERS[$tag]) ) return false;
    $BUG_TIMERS[$tag] = array(($BUG_TIMERS[$tag] || 0), microtime(true));
}
